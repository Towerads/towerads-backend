...
// ---- DB ----
const pool = new Pool({
  host: process.env.POSTGRES_HOST || "towerads_postgres",
...
async function getTowerPercent(placement_id) {
  const r = await pool.query(
    `SELECT traffic_percentage
     ...
     WHERE placement_id = $1 AND network = 'tower' AND status = 'active'
     ORDER BY priority DESC
     LIMIT 1`,
    [placement_id]
  );
  return r.rowCount ? r.rows[0].traffic_percentage : 0;
}
...
async function pickAd(placement_id, ad_type) {
  const r = await pool.query(
    `SELECT id, ad_type, media_url, click_url, duration
     FROM ads
     WHERE placement_id = $1 AND ad_type = $2 AND status = 'active'
     ORDER BY priority DESC
     LIMIT 1`,
